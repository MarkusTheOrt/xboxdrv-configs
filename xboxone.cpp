...
struct VIDException : public std::exception
{
    ...
    {
        return "VID not found in /proc/bus/input/devices, please make sure your Xbox elite gamepad is connected";
    }
};
...
void create_pid_file() {
    std::string pid_file = "/tmp/xboxone.pid";
    pid_t pid = getpid();
...
int main(int argc, char* argv[])
{
    ...
    {
        create_pid_file();
        std::string event_path = get_device("045e", "02ea");
        if (argc < 2) {
            execvp("xboxdrv", 
            {"xboxdrv", 
            "-o", event_path.c_str(), 
            ...
            "--evdev-keymap", 
            "BTN_SOUTH=A,BTN_EAST=B,BTN_NORTH=X,BTN_WEST=Y,BTN_START=start,BTN_MODE=guide,BTN_SELECT=back,BTN_TL=LB,BTN_TR=RB,BTN_TL2=LT,BTN_TR2=RT,BTN_THUMBL=TL,BTN_THUMBR=TR",
            nullptr}
            );
        }
        else {
