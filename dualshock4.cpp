...
struct VIDException : public std::exception
{
    ...
    {
        return "VID not found in /proc/bus/input/devices, please make sure your dualshock is connected";
    }
};
...
void create_pid_file() {
    std::string pid_file = "/tmp/dualshock.pid";
    pid_t pid = getpid();
...
int main(int argc, char* argv[])
{
    ...
    {
        create_pid_file();
        std::string event_path = get_device("054c", "05c4");
        if (argc < 2) {
            execvp("xboxdrv", 
            {"xboxdrv", 
            "-o", event_path.c_str(), 
